...
def category() -> IncomeCategory:
    return IncomeCategory("Salary", ScheduledIncomePlan())
...
def test_that_expected_income_before_start_date_of_today_is_0(category: IncomeCategory):
    assert category.income_plan.monthly_expected_income(category, 4, 2025) == 0
...
def test_that_expeted_income_in_april_is_0_with_repetiontion_2_months_and_start_march(
    category: IncomeCategory,
):
    plan = ScheduledIncomePlan(500, repetition=Duration(months=2), start_date=Date(2025, 3, 2))

    assert plan.monthly_expected_income(category, 4, 2025) == 0


def test_one_time_income_on_start_month(category: IncomeCategory):
    ...
    assert plan.monthly_expected_income(category, 5, 2025) == 750
...
def test_one_time_income_not_on_start_month(category: IncomeCategory):
    ...
    assert plan.monthly_expected_income(category, 6, 2025) == 0
...
def test_recurring_income_multiple_occurrences_in_month(category: IncomeCategory):
    ...
    # May has payments on 2025-05-05 and 2025-05-20
    assert plan.monthly_expected_income(category, 5, 2025) == 400
...
def test_recurring_income_respecting_end_date(category: IncomeCategory):
    ...
    # May is still within end_date, should get payment
    assert plan.monthly_expected_income(category, 5, 2025) == 300
    # June is after end_date, should be 0
    assert plan.monthly_expected_income(category, 6, 2025) == 0
